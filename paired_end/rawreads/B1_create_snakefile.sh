...
for prj_folder in PRJNA*; do
    if [[ -d "$prj_folder" ]]; then  # Ensure it's a directory
        echo -e "${CYAN}Processing $prj_folder...creating Snakefile in $prj_folder${RESET}"
	
	# Detect all unique sample IDs from fastq.gz files
        samples=($(ls "$prj_folder"/*.fastq.gz | sed -E 's#.*/(SRR[0-9]+).*#\1#' | sort -u))

        # Define the Snakefile path inside the bioproject folder
        snakefile="$prj_folder/Snakefile"

        # Start writing the Snakefile
        cat > "$snakefile" <<EOL
...
EOL
        ...
        # Append the full workflow to the Snakefile
        cat >> "$snakefile" <<'EOL'
